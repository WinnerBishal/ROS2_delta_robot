...
def image2Dworld3D(im_P):
    robot_coords = np.array([[-237.4, 34.0], [-240.4, -80.0], [-18.4,  42], [-31.4,  -95]])  
...
def send_command(ser, calibrate_motor=None, m1=None, m2=None, m3=None, gripper=None):
    ...
    if calibrate_motor is not None:
        command_parts.append(f"C:{calibrate_motor}")
...
def commander(ser, commands):
    # Send Starting Flag
    send_command(ser, calibrate_motor=123)
    time.sleep(0.2)
    for Ms in commands:
        print("here\n")
        M1 = angleToSteps(Ms[0])
        M2 = angleToSteps(Ms[1])
        M3 = angleToSteps(Ms[2])
        G = Ms[3]
        send_command(ser, m1 = M1, m2 = M2, m3 = M3, gripper = G)
        # time.sleep(0.2)
        while True:
            response = ''
        ...
        print(f"Sent Command: M1: {M1}, M2: {M2}, M3: {M3}, G: {G}")
    time.sleep(0.2)
    
    # Send ending flag
    send_command(ser, calibrate_motor=456)

    response = ''
    while ser.in_waiting > 0:
        response += ser.read().decode('utf-8').strip()
        # print(f"Arduino says: {response}")
        break
            
def angleToSteps(angle):
    return int(angle * 1600 / 360)
...
def GetObjects(frame, model_path):
    model = YOLO(model_path)
    results = model.predict(frame, max_det = 20)
    object_centers = []
      
    for box in results[0].boxes.xyxy:
        object_center = [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2]
        object_centers.append(np.array([x.cpu() for x in object_center]).reshape(-1, 2))

    cl_ids = results[0].boxes.cls.tolist()   
    all_objects = {}
    for i, label in enumerate(results[0]):
        all_objects[f'Obj{i}'] = {
            'label': label.names[cl_ids[i]],
            'location': object_centers[i],
            'rectangle': results[0].boxes.xyxy[i]
        }
